...
class WorldGenerator implements Serializable{
	...
	void init(){
		...
		_LavaBlock=new StaticBlock(LavaBlock.getInstance());
		_WaterBlock=new StaticBlock(WaterBlock.getInstance());
...
abstract class GroundGen implements Serializable{
	...
	void gen(Block[]bs,WorldGenerator g){
		int r=g.stone_y+g.dirt_d;
		for(int y=r;y<g.getWaterY();++y)bs[y]=_WaterBlock;
	}
}
...
private static final long serialVersionUID=1844677L;
	...
	void gen(WorldGenerator g,Block[] bs){
		...
		yv=g.yv+rnd(-0.1,0.1);
		int l=max(g.lava_y,rf2i(y-h)),r=min(g.stone_y,rf2i(y+h));
		for(int y=l;y<r;++y)if(bs[y].rootBlock().getClass()==StoneBlock.class&&rnd()<rou)bs[y]=_CoalOreBlock;
	}
}
...
private static final long serialVersionUID=1844677L;
	...
	void gen(WorldGenerator g,Block[] bs){
		...
		if(l<r){
			Block b=bs[r-1].rootBlock();
			if(rnd()<0.03&&b.getClass()==EnergyStoneOreBlock.class){
				bs[r]=new EnergyPlantBlock();
			}
		}
	}
}
