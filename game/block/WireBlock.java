...
public class WireBlock extends CircuitBlock{
	...
	public boolean onCheck(int x,int y){
		...
		if((tp&l_in)!=0)xe1=max(xe1,World.cur.get(x-1,y).energyValR());
		if((tp&r_in)!=0)xe1=max(xe1,World.cur.get(x+1,y).energyValL());
		if((tp&d_in)!=0)ye1=max(ye1,World.cur.get(x,y-1).energyValU());
		if((tp&u_in)!=0)ye1=max(ye1,World.cur.get(x,y+1).energyValD());
	...
	public int energyValL(){return (tp&lr_in)==l_in?0:xe;}
	public int energyValR(){return (tp&lr_in)==r_in?0:xe;}
	public int energyValD(){return (tp&ud_in)==d_in?0:ye;}
	public int energyValU(){return (tp&ud_in)==u_in?0:ye;}
	...
	public boolean ins(int x,int y,int v,int vs){
		if((vs&tp)==0){
			...
			tp=_tp;
			if(tp==lr_in)World.cur.setCircuit(x,y,new FastWireBlockH(block));
			if(tp==ud_in)World.cur.setCircuit(x,y,new FastWireBlockV(block));
			if(tp==(lr_in|ud_in))World.cur.setCircuit(x,y,new FastWireBlockC(block));
