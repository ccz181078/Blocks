...
public class GrassBlock extends SingleBlockPlantType{
	...
	public boolean onUpdate(int x,int y){
		...
		if(World.cur.get(x,y-1).rootBlock().getClass()!=DirtBlock.class){
			World.cur.setAir(x,y);
			return true;
		}
		...
		if(light_v>4&&tp>=1&&rnd()<0.3){
			...
			if(World.cur.get(x1,y1-1).rootBlock().getClass()==DirtBlock.class){
