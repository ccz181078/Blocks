...
public class EnergyFurnaceBlock extends CraftHelperBlock implements BlockWithUI,EnergyContainer{
	...
	public boolean onUpdate(int x,int y){
		...
		EnergyContainer ec1=ec.get();
		if(ec1==null)return false;
		SingleItem si[]=ecs.toArray();
		Item it=si[rndi(0,si.length-1)].get();
		if(it==null||!(it instanceof EnergyContainer))return false;
		EnergyContainer ec2=(EnergyContainer)it;
		int c=rndi(0,Math.min(ec1.getEnergy(),ec2.resCap())/16);
		ec1.loseEnergy(c);
