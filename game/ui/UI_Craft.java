...
public class UI_Craft extends UI_ItemList{
	...
	public UI_Craft(Craft[] _cls,float _x){
		super(_x,0,4,5,ItemList.craftList(_cls),null,true);
	}
	...
	protected void onDraw(Canvas cv){
		...
		//float v=1-(pl.craft_ft-World.cur.time)*1f/pl.cur_craft.cost.time;
	...
	public void update(){
		super.update();
		if(pl.last_pressed_ui==this&&is_craft){
			getCraft().start(pl);
		}
	}
	
	transient boolean is_craft=false;
	
	@Override
	protected boolean onTouch(float tx,float ty,int tp){
		if(super.onTouch(tx,ty,tp)){
			is_craft=false;
			return true;
		}
