...
public abstract class Human extends Agent implements Crafter,Launcher{
	...
	public AttackFilter getAttackFilter(){
		return new AttackFilter(){public Attack transform(Attack a){
			if(a!=null){
				...
				if(it!=null&&it instanceof Shield)((Shield)it).transform(a);
	...
	public void clickAt(double tx,double ty){
		if(abs(tx-x)<TOUCHABLE_XD&&abs(ty-y)<TOUCHABLE_YD){
			...
			if(it==null)new AirBlock().clickAt(tx,ty,this);
	...
	public void update(){
		...
		if(des_flag||attack_flag){
			...
			for(Agent a:World.cur.getNearby(x1,y,w1,h1,false,false,true).agents){
				if(a==this)continue;
