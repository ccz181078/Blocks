...
public class Script{
	...
	//script runtime:
	static Object[] var_stack,params;
	...
	static{
		...
		ops.put("#",new GetParamOp());
		ops.put("@",new LambdaOp());
	...
	public static Runnable run(String fname){
		return run(fname,new Object[0]);
	}
	public static Runnable run(String ss[]){
		String fname=ss[0];
		Object pars[]=new Object[ss.length-1];
		for(int i=0;i<pars.length;++i){
			pars[i]=ss[i+1];
		}
		return run(fname,pars);
	}
	public static Runnable run(String fname,Object pars[]){
		try{
			StringBuffer sb=new StringBuffer();
			sb.append("(\n");
			util.SerializeUtil.readStringFromFile(sb,debug.Log.SCRIPT_DIR+fname+".txt");
			sb.append("\n)\n");
			debug.Log.showTime(false);
			final rvalue r=compile(sb.toString());
			return () -> {
				try{
					init();
					params=pars;
			...
			System.err.println("failed to execute: "+fname);
		}catch(ScriptException e){
			debug.Log.i(e.info);
			if(e.e!=null)debug.Log.i(e.e);
			debug.Log.i(e);
...
class ValStack implements Iterable<Expr>{
...
class ScriptException extends Exception{
	...
	final int L,R;
	final String info;
	ScriptException(int _L,int _R,int __){L=_L;R=_R;e=null;info="";}
	ScriptException(int _L,int _R){L=lrs.get(_L*2);R=lrs.get(_R*2+1);e=null;info="";}
	ScriptException(int _L,int _R,Exception _e){L=lrs.get(_L*2);R=lrs.get(_R*2+1);e=_e;info="";}
	ScriptException(int _L,int _R,String _s){L=lrs.get(_L*2);R=lrs.get(_R*2+1);info=_s;e=null;}
}

class ReturnException extends ScriptException{
	final Object v;
