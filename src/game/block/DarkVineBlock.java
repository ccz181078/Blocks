...
public final class DarkVineBlock extends PlantType{
	...
	public void onPress(int x,int y,game.item.Item it){
		if(damage==0&&(it instanceof game.item.Scissors)){
			...
			if(rnd(maxDamage())<1){
				World._.setAir(x,y);
				new DarkVineBlock(tp).asItem().drop(x,y);
			}
			return;
		}
	...
	public boolean onUpdate(int x,int y){
		...
		if(dirt_v>0.05)dirt_v-=0.05;
		repair(0.1,0);
		if(tp==1&&World._.get(x,y-1).rootBlock().getClass()==DarkSandBlock.class)dirt_v=Math.min(5,dirt_v+1f);
		int x1=x,y1=y;
