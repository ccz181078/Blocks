...
public class WireBlock extends CircuitBlock{
	...
	public boolean onCheck(int x,int y){
		...
		if((tp&l_in)!=0)xe1=max(xe1,World._.get(x-1,y).energyValX()-1);
		if((tp&r_in)!=0)xe1=max(xe1,World._.get(x+1,y).energyValX()-1);
		if((tp&u_in)!=0)ye1=max(ye1,World._.get(x,y+1).energyValY()-1);
		if((tp&d_in)!=0)ye1=max(ye1,World._.get(x,y-1).energyValY()-1);
		...
		if(xe1!=xe||ye1!=ye)World._.check4(x,y);
		xe=xe1;ye=ye1;
		return false;
	}
