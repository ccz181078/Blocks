...
public abstract class Block implements Serializable,Cloneable{
	...
	//比较两个方块在作为物品时是否可合并成一项
	public boolean cmpType(Block b){
		return getClass()==b.getClass();
	}
	...
	public void des(int x,int y,int v){
		damage+=v;
		if(damage>=maxDamage())World._.checkBlock(x,y);
	}
	//被破坏/覆盖时调用
	...
	public Item heatingProduct(boolean in_furnace){return Item.air();}
	...
	public void onFireUp(int x,int y){
		...
		if(v>0){
			...
			if(World._.get(x1,y1).isCoverable())World._.place(x1,y1,new FireBlock());
		}
	}
	...
	public void rigidBodyHitTest(int x,int y,Entity ent){
		...
		//if(ent instanceof Player)android.util.Log.i("block touch pl",xd+","+yd+" WH:"+W+","+H+" ev:"+ent.xv+","+ent.yv);
