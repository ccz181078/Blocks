...
public abstract class Agent extends Entity{
	...
	public void add(){
		World._.newAgent(this);
	}
	public boolean cadd(){
		if(!World._.noBlock(x,y,width(),height()))return false;
		//debug.Log.i("add:"+World._.RX(x)+","+World._.RY(y)+":"+getClass().getSimpleName());
		spawn_time=last_seen_time=World._.time;
		add();
		return true;
	}
	...
	public void action(){//应用动作
		...
		double xp_a=0.005;
		if(World._.weather==Weather._energystone)xp_a=0.03;
		xp=min(maxXp(),xp+xp_a);
	}
	public void update(){
		super.update();
		is_attacked=max(0,is_attacked-1);
		if(!removed&&chkRemove(World._.time-last_seen_time))remove();
	}
	public boolean chkRemove(long t){//t帧未被绘制过，检查是否移除
		return t>=30*60;
	}
	public void desBlock(){//破坏方块
		World._.get(des_x,des_y).onPress(des_x,des_y,Item.air());
	}
	public boolean destroyable(int dx,int dy){
		double xd=abs(dx+0.5-x),yd=abs(dy+0.5-y);
		return xd<=4&&yd<=4&&(xd<0.5+width()&&yd<0.5+height()||World._.destroyable(dx,dy));
	}
	public final void before_draw(Canvas cv){
		last_seen_time=World._.time;
