...
class DesBlock extends Goal{
	...
	public Goal update(){
		...
		int mv=0;
		if(ba.exist()){
			if(World._.destroyable(ba.x,ba.y))
			for(SingleItem s:items.toArray())if(!s.isEmpty()){
				Item it=s.get();
				int v=0;
...
class MoveToAgent extends Goal{
	...
	public Goal update(){
		...
		if(in_wall&&rnd()<0.5){
			BlockAt ba=World._.get1(f2i(x+rnd(-width(),width())),f2i(y+rnd(-height(),height())));
		...
		if(xdir!=0&&xdep!=0&&rnd()<0.5){
			...
			BlockAt ba=World._.get1(f2i(x+(width()+0.1)*xdir*rnd()),f2i(y+height()*rnd(0.2,1.1)));
			if(ba.block.isSolid())return new DesBlock(this,ba);
		}
		
		if(ydep>0&&rnd()<0.7){
			BlockAt ba=World._.get1(f2i(x+rnd(-width(),width())),f2i(y+height()*rnd(0.5,1.1)));
			if(ba.block.isSolid())return new DesBlock(this,ba);
		}
		
		if(ydir<0&&ydep<0&&rnd()<0.5){
			BlockAt ba=World._.get1(f2i(x+rnd(-width(),width())),f2i(y-height()));
			if(ba.block.isSolid())return new DesBlock(this,ba);
		}
